...
read_data_name_up = 'Force-Up'
read_data_name_down = 'Force-Down'
...
read_data_up = np.zeros((num_vertices, dimensions))
read_data_down = np.zeros((num_vertices, dimensions))
...
read_data_up = vertices.copy()
read_data_down = vertices.copy()
write_data = vertices.copy()

vertex_ids = interface.set_mesh_vertices(mesh_id, vertices)
read_data_id_up = interface.get_data_id(read_data_name_up, mesh_id)
read_data_id_down = interface.get_data_id(read_data_name_down, mesh_id)
...
while interface.is_coupling_ongoing():
    ...
    if interface.is_read_data_available():
        read_data_up = interface.read_block_vector_data(read_data_id_up, vertex_ids)
        read_data_down = interface.read_block_vector_data(read_data_id_down, vertex_ids)

    print("<read data up>")
    print(read_data_up[0:3,:])
    print("</read data up>")
    if iteration % 1 == 0:
    	next += 1
